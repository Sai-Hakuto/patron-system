  ...
  hooks = {
    onInit = function(self)
      ...
      self.cardGrid = self:CreateCardGrid(self.frame, {
        top    = 110,
        bottom = 80,
        cellW  = 72,  -- Увеличенный размер для вкладки
        cellH  = 72,  -- Увеличенный размер для вкладки
        cols   = 4,   -- Меньше колонок для больших иконок
        gapX   = 8,   -- Немного больший промежуток
        gapY   = 8,   -- Немного больший промежуток
      })

      -- ================== ПАНЕЛЬ АКТИВНЫХ БЛАГОСЛОВЕНИЙ ==================  
      
      -- Создаем визуальный контейнер для панели благословений
      self.activePanelContainer = CreateFrame("Frame", nil, self.frame, "BackdropTemplate")
      self.activePanelContainer:SetBackdrop({
        bgFile="Interface\\Buttons\\WHITE8x8",
        edgeFile="Interface\\Tooltips\\UI-Tooltip-Border",
        tile=false, edgeSize=8,
        insets={left=4,right=4,top=4,bottom=4},
      })
      self.activePanelContainer:SetBackdropColor(0.08, 0.08, 0.1, 0.85)  -- Как в client_solution
      self.activePanelContainer:SetBackdropBorderColor(0.3, 0.3, 0.35, 0.8)
      
      self.activeBar = self:CreateSlotBar(self.activePanelContainer, 12, {
        title = "Active Blessings:",
        size = 36,  -- Размер как у спеллов
        gap = 4,    -- Меньший промежуток
      }, 2, 6)  -- 2 строки по 6 слотов
      
      -- Размещаем слоты по центру контейнера
      self.activeBar.frame:SetPoint("CENTER", self.activePanelContainer, "CENTER", 0, -8)
      
      -- Размещаем контейнер внизу главного окна
      local containerWidth = 6 * 36 + 5 * 4 + 16  -- 6 слотов + промежутки + отступы
      local containerHeight = 2 * 36 + 1 * 4 + 24 + 16  -- 2 строки + промежуток + заголовок + отступы
      self.activePanelContainer:SetSize(containerWidth, containerHeight)
      self.activePanelContainer:SetPoint("BOTTOM", self.frame, "BOTTOM", 0, 15)
...
function NS.BlessingWindow:AddBlessingToSlotSilent(blessing)
  ...
  for i, slot in ipairs(self.activeBar.slots) do
    if not slot.__blessing then
      ...
      -- Добавляем highlight как у спеллов
      slot:SetHighlightTexture("Interface\\Buttons\\ButtonHilight-Square")
      slot:GetHighlightTexture():SetBlendMode("ADD")
      
      -- Отключаем изменение цвета при нажатии - устанавливаем одинаковые текстуры
      slot:SetNormalTexture(blessing.icon)
      slot:SetPushedTexture(blessing.icon)
      
      -- Добавляем tooltip для слота
      slot:SetScript("OnEnter", function(self)
        if self.__blessing then
          GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
          GameTooltip:SetText(self.__blessing.name, 1, 1, 1)
          if self.__blessing.description then
            GameTooltip:AddLine(self.__blessing.description, 1, 0.82, 0, true)
          end
          GameTooltip:Show()
        end
      end)
...
function NS.BlessingWindow:RenderCategory(category)
  ...
  for _, blessing in ipairs(blessings) do
    self.cardGrid:AddCard(blessing, function(card, data)
      -- Убираем backdrop полностью для чистого вида
      card:SetBackdrop(nil)
      card:SetBackdropColor(0, 0, 0, 0)
      card:SetBackdropBorderColor(0, 0, 0, 0)
      
      -- Создаем иконку спелла по образцу client_solution
      local icon = card:CreateTexture(nil, "ARTWORK")
      icon:SetAllPoints(card)
      icon:SetTexCoord(0.07, 0.93, 0.07, 0.93)
      icon:SetTexture(data.icon)
      
      -- Настраиваем текстуры как в спеллах
      card:SetNormalTexture(data.icon)
      card:SetPushedTexture(data.icon)
      card:SetHighlightTexture("Interface\\Buttons\\ButtonHilight-Square")
      card:GetHighlightTexture():SetBlendMode("ADD")
      
      -- Убираем текст под иконкой для чистоты (как в спеллах)
      card.__data = data
      
      -- Добавляем tooltip с названием и описанием
