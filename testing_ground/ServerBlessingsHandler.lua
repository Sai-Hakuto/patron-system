...
local BlessingsValidator = {}

-- Проверка существования благословения
function BlessingsValidator.ValidateBlessingExists(player, blessingID)
...
-- Проверка и получение цели
function BlessingsValidator.ValidateAndGetTarget(player, info)
    ...
    return true, finalSpellTarget
end

-- Проверка возможности атаки цели
function BlessingsValidator.ValidateOffensiveTarget(player, targetUnit, info)
    ...
    -- 2. Проверка дружебности через GetFriendlyUnitsInRange
...
-- Проверка состояния игрока (аура)
function BlessingsValidator.ValidatePlayerState(player, info)
...
-- Проверка кулдауна
function BlessingsValidator.ValidateCooldown(player, blessingID, info)
...
function BlessingsValidator.ValidateAndConsumeCost(player, info)
...
-- Сбор враждебных целей в радиусе для AoE (как в оригинальном коде)
function BlessingsValidator.GetHostileTargetsInRadius(caster, centerX, centerY, centerZ, radius)
    if not (caster and caster:IsInWorld()) then return {} end
    
    -- Оригинальная логика: радиус поиска вокруг игрока
    local playerDistance = caster:GetDistance(centerX, centerY, centerZ) or 0
    local searchRadius = math.min(120, playerDistance + radius + 10)
    local hostileUnits = (caster.GetUnfriendlyUnitsInRange and caster:GetUnfriendlyUnitsInRange(searchRadius)) or {}
    
    print(("[BlessingUI - Server DEBUG] AoE search: playerDist=%.1f, searchRadius=%.1f, pool=%d"):format(playerDistance, searchRadius, #hostileUnits))
    
    local validTargets = {}
    for _, unit in ipairs(hostileUnits) do
        -- Проверяем дистанцию от центра AoE (не от игрока!)
        if unit and unit:IsInWorld() and unit:IsAlive() then
            local distanceFromCenter = unit:GetDistance(centerX, centerY, centerZ) or 1e9
            if distanceFromCenter <= radius then
                -- Дополнительная проверка атакуемости
                local success, isTargetable = pcall(unit.IsTargetableForAttack, unit)
                if success and isTargetable then
                    validTargets[#validTargets + 1] = unit
                end
            end
        end
    end
    
    print(("[BlessingUI - Server DEBUG] AoE valid targets: %d"):format(#validTargets))
    return validTargets
end

-- Проверка валидности цели для AoE урона (упрощенная, так как основная логика в GetHostileTargetsInRadius)
function BlessingsValidator.IsValidAoETarget(unit, centerX, centerY, centerZ, radius)
    -- Эта функция оставлена для совместимости, основная логика теперь в GetHostileTargetsInRadius
    if not (unit and unit:IsInWorld() and unit:IsAlive()) then
        return false
    end
    
    local distance = unit:GetDistance(centerX, centerY, centerZ) or 1e9
    return distance <= radius
end

-- Расчет урона за тик AoE на основе количества целей
function BlessingsValidator.CalculateAoETick(baseSingleDamage, targetCount, info)
    if targetCount == 0 then
        return 0 -- Нет целей - нет урона
    end
    
    local capTargets = tonumber(info.aoe_cap_targets) or AOE_CAP_DEFAULT
    local aoeMult = 0.75 -- Множитель AoE урона
    
    -- Используем существующую логику расчета
    return AoE_tick_from_single(baseSingleDamage, targetCount, capTargets, aoeMult)
end
...
function BlessingsHandler.RequestBlessing(player, data)
    ...
    local success_pcall, result_pcall = pcall(function()
        ...
        -- Проверка существования благословения
        local isValid, info = BlessingsValidator.ValidateBlessingExists(player, blessingID)
        if not isValid then return end

        -- === ЛОГИКА ОБРАБОТКИ ЦЕЛИ ===
        -- Проверка и получение цели
        local isTargetValid, finalSpellTarget = BlessingsValidator.ValidateAndGetTarget(player, info)
        if not isTargetValid then return end
        
        -- Проверка возможности атаки цели (если это атакующее заклинание)
        if info.is_offensive and info.requires_target then
            if not BlessingsValidator.ValidateOffensiveTarget(player, finalSpellTarget, info) then
                return
            end
        end
        ...
        -- === ПРОВЕРКИ ИГРОКА ===
        -- Проверка состояния игрока (аура)
        if not BlessingsValidator.ValidatePlayerState(player, info) then
            return
        end
		
		-- Проверка кулдауна
        if not BlessingsValidator.ValidateCooldown(player, blessingID, info) then
            return
        end

        -- Проверка и списание стоимости
        if not BlessingsValidator.ValidateAndConsumeCost(player, info) then
            return
        end

        -- === ПРИМЕНЕНИЕ ЭФФЕКТА ===
        if info.is_aoe then
            DoAoE(player, finalSpellTarget, info)
        elseif info.is_offensive then
            DoSingle(player, finalSpellTarget, info)
        else
...
function DoBuff(caster, target, info)
    ...
    if info.spell_id > 0 then
	    pcall(caster.CastSpell, caster, target, info.spell_id, true) -- Unit:CastSpell(target, spell, triggered)
	    -- triggered=true: мгновенно, без стоимости/GCD. Возвращает ничего.
	end
	-- 2) Гарантированно повесим ауру, если её нет (для воина)
	local existing = target:GetAura(info.spell_id)  -- вернёт Aura или nil
		if not existing then
	    	local ok, auraObj = pcall(caster.AddAura, caster, info.spell_id, target) -- Unit:AddAura(spell, target)
...
function DoSingle(caster, target, info)
  local bp = PatronCalc_SingleBudget(caster, info)
  local bp0, bp1, bp2 = build_bp_triplet(info, bp)
  local tickId = info.spell_tick_id or info.spell_id
  if not tickId then caster:SendBroadcastMessage("spell_tick_id не указан."); return end
  local ok = pcall(caster.CastCustomSpell, caster, target, tickId, true, bp0, bp1, bp2)
end

function DoAoE(caster, centerUnit, info)
  if not (caster and centerUnit and caster:IsInWorld() and centerUnit:IsInWorld()) then 
    print("[BlessingUI - Server DEBUG] AoE aborted: invalid caster or centerUnit")
    return 
  end

  local cx, cy, cz = centerUnit:GetX(), centerUnit:GetY(), centerUnit:GetZ()
  local radius      = tonumber(info.radius) or 8.0
  local tickMs      = tonumber(info.tick_ms) or 500
  local durationMs  = tonumber(info.duration_ms) or 4000
  local repeats     = math.max(1, math.floor(durationMs / tickMs))
  local tickId      = tonumber(info.spell_tick_id) or 2136

  -- Ограничение на количество тиков для предотвращения переполнения
  if repeats > 50 then
    print(("[BlessingUI - Server DEBUG] AoE warning: too many repeats (%d), capping to 50"):format(repeats))
    repeats = 50
  end

  if info.spell_id and info.spell_id > 0 then
    pcall(caster.CastSpellAoF, caster, cx, cy, cz, info.spell_id, true) -- AoE визуал
  end

  local singleBase = PatronCalc_SingleBudget(caster, info)
  print(("[BlessingUI - Server DEBUG] AoE start center=(%.1f,%.1f,%.1f) R=%.1f ticks=%d singleBase=%d"):format(cx, cy, cz, radius, repeats, singleBase))

  local tickN, totalCasts = 0, 0
  local maxTargetsPerTick = 10 -- Ограничение целей за тик

  local function onTick(eventId, _delay, repeatsLeft, wo)
    -- Простая проверка как в старом коде
    ...
    -- Простой поиск целей как в старом коде
    ...
    for _, u in ipairs(pool) do
      if u and u:IsInWorld() and u:IsAlive() and (u:GetDistance(cx, cy, cz) or 1e9) <= radius then
        -- Рассчитаем урон для этого тика (как в новой логике, но проще)
        local dmgTick = AoE_tick_from_single(singleBase, #pool, info.aoe_cap_targets or 8, 0.75)
        local bp0, bp1, bp2 = build_bp_triplet(info, dmgTick)
        
        local ok = pcall(wo.CastCustomSpell, wo, u, tickId, true, bp0, bp1, bp2)
        if ok then 
          hits = hits + 1
    ...
    if hits > 0 or tickN == 1 or tickN % 4 == 0 then
      print(("[BlessingUI - Server DEBUG] AoE tick=%d pdist=%.1f searchR=%.1f pool=%d hits=%d"):format(tickN, pdist, searchR, #pool, hits))
    end

    if repeatsLeft and repeatsLeft <= 1 then
      print(("[BlessingUI - Server DEBUG] AoE done ticks=%d totalCasts=%d"):format(tickN, totalCasts))
    end
  end

  -- Регистрация события с защитой
  local success = pcall(caster.RegisterEvent, caster, onTick, tickMs, repeats)
  if not success then
    print("[BlessingUI - Server DEBUG] AoE failed to register event")
  end
end
...
function PatronCalc_SingleBudget(caster, info)
  ...
  local autoDPS = weapDPS + (ap / AP_PER_DPS)  -- retail формула AP→DPS :contentReference[oaicite:12]{index=12}
...
-- Eluna: Player:GetEquippedItemBySlot(15) → Item; Item:GetDamageInfo(slot); Item:GetSpeed() (мс)
